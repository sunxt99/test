...
def _summarize_group(name: str, reqs: list[Request], t_end: float) -> str:
    ...
    lines.append("  TPOT_token/s: " +
                 f"mean={_fmt_ms(float(np.mean(tpot)))}, p10={_fmt_ms(_pct(tpot, 100-10))}, p50={_fmt_ms(_pct(tpot, 100-50))}, p90={_fmt_ms(_pct(tpot, 100-90))}")
    lines.append("")
    return "\n".join(lines)
...
def summarize_metrics(res_list: List[SimulationResult]) -> str:
    ...
    lines.append(f"finished_total={n}, throughput_total={n / t_end:.6f} req/s")
    lines.append("")
